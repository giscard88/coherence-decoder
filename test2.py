...
if __name__ == '__main__':
    ...
    train_set = load_bbci_data(
        train_filename, low_cut_hz=0)
    print (type(train_set.X),train_set.X.shape)
    print (train_set.y,len(train_set.y))
    tr, elc, ts_n=train_set.X.shape    

    

    coherence={}
    data=[]
    for t in range(1): #range(tr):
        #print (train_set.X[t,:,:])
        #print (train_set.X[t,:,:].shape)
        data.append(train_set.X[t,0,:])
        data.append(train_set.X[t,0,:])
        data=np.array(data)
        print (data.shape)

        time_series = ts.TimeSeries(data, sampling_rate=250.0,time_unit='s')
